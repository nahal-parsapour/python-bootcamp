# from asyncio import log
#
# git --version
# git config
# git config --l
# git config list
# git config --global --get core.editor
# git config --global user.name ("...")
# git config --global user.email "...@gmail.com"
# config --global core.editor "code --wait"
# git config --global --edit
# git config --system --edit
#
# git init
# cd .git
# ls
# code config
# git status
#
# git add 12\ Web\ Foundations/
# git commit -m "..."
# git log
#
# code .gitignore
#                 .venv/
#                 *.log
# https://www.toptal.com/developers/gitignore
#
# git add ...
# git add .      # all changed files → staging area
# git commit --amend     # merge with previous commit
#
# # copy a repo on git and paste it on your device:
# git clone https://github.com/nahal-parsapour/python-bootcamp.git
#
# # create repo on git, connect repo on device to → repo on git:
# git remote add origin git@github.com:nahal-parsapour/python-bootcamp.git
# # or
# git remote add origin https://github.com/nahal-parsapour/python-bootcamp.git
#
# git remote
# git remote -v
#
# # commit structure: hash, message, parent
#
# git push origin main
# # create ssh key for remember name & user-id
#
# git branch
# git branch nahal
# git switch nahal
# git branch -r
#
# # for merge:
# git switch main
# git merge nahal  # → commit with 2 parents / merge conflict
#
# # change name of "main" branch:
# git branch -M master
# git push origin master
#
# # commit on git → device
# git fetch origin main
#
# # fetch + merge: pull
# git pull origin main
#
#

